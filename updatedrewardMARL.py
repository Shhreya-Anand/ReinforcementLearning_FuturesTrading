...
class MultiAgentSystem:
    ...
    def MaybeDecide(self):
        ...
        if self.risk_manager.max_equity is None:
            self.risk_manager.max_equity = equity
        else:
            self.risk_manager.max_equity = max(self.risk_manager.max_equity, equity)
        
        if self.risk_manager.max_equity > 0:
            self.risk_manager.drawdown = (self.risk_manager.max_equity - equity) / self.risk_manager.max_equity
        else: 
          self.risk_manager.drawdown = 0.0
...
class VIXTraderAgent:
    ...
    def __init__(self, algorithm: QCAlgorithm):
        ...
        self.lambda_vol = 5.0  # penalty for realized volatility
        self.lambda_dd = 3.0    # penalty for drawdown

        # Rolling window for volatility calculation
        self.returns_window = deque(maxlen=40)  # stores recent equity returns
        self.prev_sigma = 0.0
    ...
    def UpdateFromReward(self, current_equity: float, train_phase: bool):
        ...
        pnl = (current_equity - self.last_equity) / self.reward_scale

        # Track equity returns for volatility calculation
        if self.last_equity > 0:
            equity_return = (current_equity - self.last_equity) / self.last_equity
            self.returns_window.append(equity_return)

        # Calculate realized volatility (sigma)
        sigma = 0.0
        if len(self.returns_window) >= 5:
            sigma = float(np.std(self.returns_window))

        # Calculate current drawdown
        drawdown = 0.0
        if hasattr(self.algorithm.multi_agent.risk_manager, 'drawdown'):
            drawdown = self.algorithm.multi_agent.risk_manager.drawdown
        # Initialize prev_sigma after warmup to avoid initialization bias
        if self.prev_sigma == 0.0 and len(self.returns_window) >= 10:
            self.prev_sigma = sigma
        # Risk-penalized reward: pnl - lambda_vol * sigma - lambda_dd * drawdown
        reward = pnl - self.lambda_vol * sigma - self.lambda_dd * drawdown
...
class ESHedgeAgent:
    ...
    def __init__(self, algorithm: QCAlgorithm, trader_agent: VIXTraderAgent):
        ...
        self.entropy_coef = 0.001
        self.critic_coef = 0.5
        self.beta = 20.0   # reward coefficient for reducing sigma
        self.gamma = 3.0    # penalty for drawdown

        # Rolling window for portfolio volatility tracking
        self.returns_window = deque(maxlen=40)
        self.prev_sigma = 0.0
    ...
    def UpdateFromReward(self, current_equity: float, train_phase: bool): #implements leanring step, both recieve same reward - important to cooperative RL
        ...
        sigma_reduction = self.prev_sigma - sigma
        self.prev_sigma = sigma
        ...
        # Volatility-reduction reward: beta * (prev_sigma - sigma) - gamma * drawdown
        reward = self.beta * sigma_reduction - self.gamma * drawdown
        reward_t = torch.tensor([reward], dtype=torch.float32, device=self.device)
